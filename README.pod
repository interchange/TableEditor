...
Table editor specific configuration is located in lib/config.yml. Configuration can also be set in schema classes.
...
Following example illustrates scenario where admin has full (all of them), analyst and secreatary have reading permission and boss has delete (and inclusive update and read). Role can be in multiple arrays.
 
	read: [analyst, secreatary]
	update: []
	delete: [boss]
	create: []
	full: [admin]
 
Leves are hierarchical and they include:
read: []
update: [read]
delete: [update]
create: [update]
...
Same thing just under certain column. Available levels are restricted, read, update.
	...
	classes:
		Users:
			columns:
				salary:
					restrict: [analyst, secreatary]			
					update: [boss]
					
=head3 Result subset permissions
 
If you want to limit records to some role, you can set conditions. 
For example showing only active (where active = 1) users to secreatary and boss. 
