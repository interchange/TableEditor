...
configuration files. This file is located at F<lib/config.yml>. You can change
the file name in the Dancer configuration with C<settings_file>:
...
Autocomplete searches on all columns by default. This can be altered with this attribute.
...
For example showing only active (where active = 1) users to secretary and boss.
